...
const SearchResults: FunctionComponent = () => {
  ...
  const page = searchParams.get(ROUTER_CONFIG.SEARCH_PAGE.SEARCH_PARAMS.PAGE);
  const name = searchParams.get(ROUTER_CONFIG.SEARCH_PAGE.SEARCH_PARAMS.NAME);
  ...
  const currentPage = Number(searchParams.get(ROUTER_CONFIG.SEARCH_PAGE.SEARCH_PARAMS.PAGE) ?? 1);
  ...
  useEffect(() => {
    if (searchNotes.isSuccess && currentPage > maximumPage) {
      setSearchParams((previousSearchParams) => ({
        ...
        [ROUTER_CONFIG.SEARCH_PAGE.SEARCH_PARAMS.NAME]: previousSearchParams.get(ROUTER_CONFIG.SEARCH_PAGE.SEARCH_PARAMS.NAME) ?? ''
      }));
    }
  ...
  const handleResetSearchParams = useImmutableCallback(() => {
    setSearchParams((previousSearchParams) => ({
      ...
      [ROUTER_CONFIG.SEARCH_PAGE.SEARCH_PARAMS.PAGE]: '1',
      [ROUTER_CONFIG.SEARCH_PAGE.SEARCH_PARAMS.NAME]: ''
    }));
  });
